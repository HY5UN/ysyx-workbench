...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = (word_t)imm<<12);
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1+imm);//addi
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, do{R(rd)=s->snpc;s->dnpc=(imm+src1)& 0xFFFFFFFEu;}while(0));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd)=Mr(src1+imm,4));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4,src2));

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, do {R(rd) = s->snpc;s->dnpc=s->pc+imm;}while(0));
