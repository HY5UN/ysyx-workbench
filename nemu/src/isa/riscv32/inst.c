...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, do{R(rd)=s->snpc;s->dnpc=(imm+src1)& 0xFFFFFFFEu;if(ftrace_enabled) ftrace_record(s->dnpc, rd);}while(0));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, do {R(rd) = s->snpc;s->dnpc=s->pc+imm;if(ftrace_enabled) ftrace_record(s->dnpc, rd);}while(0));
