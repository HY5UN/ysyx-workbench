...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, do{R(rd)=s->snpc;s->dnpc=(imm+src1)& 0xFFFFFFFEu;if(ftrace_enabled) ftrace_record(s->pc, rd);}while(0));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd)=Mr(src1+imm,4));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = src1 <imm ? 1 : 0);
  INSTPAT("0000000 ????? ????? 011 ????? 00000 11", slli   , I, R(rd) = src1 << (imm & 0x1f));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, do {R(rd) = s->snpc;s->dnpc=s->pc+imm;if(ftrace_enabled) ftrace_record(s->pc, rd);}while(0));
