...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, do{R(rd)=s->snpc;s->dnpc=(imm+src1)& 0xFFFFFFFEu;IFDEF(CONFIG_ITRACE,ftrace_record(s->pc, s->dnpc, rd, rs1, false)) ;}while(0));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, do {R(rd) = s->snpc;s->dnpc=s->pc+imm;IFDEF(CONFIG_ITRACE,ftrace_record(s->pc, s->dnpc, rd,rs1, true));}while(0));
